...
func (suite *ServiceOutputRepositorySuite) TestSaveServiceOutputWhenIsANewServiceOutput() {
	...
	suite.NoError(err)
	err = suite.repo.SaveServiceOutput(serviceOutput, suite.Collection)
	suite.NoError(err)
}
...
func (suite *ServiceOutputRepositorySuite) TestFindAllByService() {
	serviceOutput1, err := entity.NewServiceOutput(map[string]interface{}{"test": "test"}, "test", map[string]interface{}{"test": "test"}, "test", "test", "file-downloader", "test", "test")
	suite.NoError(err)
	err = suite.repo.SaveServiceOutput(serviceOutput1, suite.Collection)
	suite.NoError(err)

	serviceOutput2, err := entity.NewServiceOutput(map[string]interface{}{"test": "test"}, "test", map[string]interface{}{"test": "test"}, "test", "test", "file-downloader", "test2", "test")
	suite.NoError(err)
	err = suite.repo.SaveServiceOutput(serviceOutput2, suite.Collection)
	suite.NoError(err)
